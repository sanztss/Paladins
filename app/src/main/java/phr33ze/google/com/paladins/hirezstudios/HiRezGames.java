...
import phr33ze.google.com.paladins.hirezstudios.instance.TierLeauge;
...
public class HiRezGames extends HttpClient {
    ...
    HiRezGames(Paladins.Platform platform, String devId, String authKey, Context context) {
        super(platform.url);
        this.devId = devId;
        this.authKey = authKey;
        this.context = context;
        this.session = new Session(platform, devId, authKey, context);
    }
    ...
    public StringData getMatchDetails(String... match_id) {
        if (match_id.length <= 10 && match_id.length > 1) {
            return get("getmatchdetailsbatch", TextUtils.join(",", match_id));
     ...
     */
    public StringData getQueueStats(String username, Paladins.Queue queue) {
     ...
     */
    public StringData getPlayerAchievements(String username) throws JSONException {
